...
CUSTOMER_FN_FIELD = "CustomerFN"

PREPARE_CONTRACT_ROLE = "Prepare Contract"
CUSTOMER_SIGN_ROLE = "Customer to Sign"

APP_URL = "http://localhost:8080"
SAMPLE_NAME = "EmbeddedSenderWithFormAndFirstSigner"
REDIRECT_BASE_URL = f"{APP_URL}/samples/{SAMPLE_NAME}"
SIGNING_URL_BASE = "https://app.signnow.com/webapp/documentgroup/signing"
...
class IndexController(SampleController):
    ...
    def handle_post(self, form_data: dict[str, Any]) -> Response:
        ...
        if action == "create_signing_url":
            return self._create_signing_url(form_data, client)
    ...
    def _create_signing_url(self, data: dict[str, Any], client) -> Response:
        dg_id = data["document_group_id"]
        signing_url = self._create_signing_link(client, dg_id)
        return JSONResponse({
            "success": True,
    ...
    def _create_document_group_from_template(self, client) -> str:
        req = DocumentGroupTemplatePostRequest(
            "Embedded Sender Form Document Group", "", None
    ...
    def _update_document_fields(self, client, dg_id: str, name: str) -> None:
        ...
        for doc_item in dg.documents or []:
            ...
            if CUSTOMER_NAME_FIELD in existing_fields:
                fields.add(Field(CUSTOMER_NAME_FIELD, name))
            elif CUSTOMER_FN_FIELD in existing_fields:
                fields.add(Field(CUSTOMER_FN_FIELD, name))
    ...
    def _extract_field_name(self, field) -> str | None:
        if not isinstance(field, dict):
            return None
        json_attrs = field.get("jsonAttributes") or field.get("json_attributes")
    ...
    def _create_embedded_sending_url(self, client, dg_id: str) -> str:
        redirect_url = f"{REDIRECT_BASE_URL}?page=signing-page&document_group_id={dg_id}"
    ...
    def _create_signing_link(self, client, dg_id: str) -> str:
        access_token = self._generate_limited_token(client, dg_id)
    ...
    def _generate_limited_token(self, client, dg_id: str) -> str:
        req = TokenPostRequest(
            USER_EMAIL,
            USER_PASSWORD,
            f"limited_signer_scope_token_for_document_group_invite/{dg_id}",
            "password",
            "",
        )
        resp = client.send(req).get_response()
        return getattr(resp, "access_token", None) or resp.access_token
